...
type FormValues = { title: string; description?: string; priority?: string; category?: string }
...
export function MaintenanceNew() {
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      setSuccessMessage('Ticket submitted.')
  ...
  return (
    <div className="px-4 sm:px-6">
      <Text variant="h1">New Ticket</Text>
      <Card className="mt-4">
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[13px] font-medium text-slate-900">Category</label>
            <select
              {...register('category')}
              disabled={isSubmitting}
              className="rounded-input border border-slate-300 bg-white px-3 py-2 text-[15px] outline-none focus:border-primary focus:ring-1 focus:ring-primary"
            >
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[13px] font-medium text-slate-900">Priority</label>
            <select
              {...register('priority')}
              disabled={isSubmitting}
              className="rounded-input border border-slate-300 bg-white px-3 py-2 text-[15px] outline-none focus:border-primary focus:ring-1 focus:ring-primary"
