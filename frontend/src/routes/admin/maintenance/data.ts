...
export async function getMaintenanceRequests(params: GetMaintenanceRequestsParams = {}): Promise<GetMaintenanceRequestsResult> {
  ...
  return await api.get(`/maintenance?${query.toString()}`)
}
...
export async function getMaintenanceRequest(requestId: string): Promise<MaintenanceRequest | undefined> {
  return await api.get(`/maintenance/${requestId}`)
}
...
export async function updateMaintenanceRequest(
  ...
  return await api.patch(`/maintenance/${requestId}`, updates)
}
...
export async function addMaintenanceRequest(input: AddMaintenanceRequestInput): Promise<{ success: boolean; id: string }> {
  return await api.post('/maintenance', input)
}
