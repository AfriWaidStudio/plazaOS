...
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  const targetFilter = parsed.data.audience === 'selected' 
    ? { role: 'tenant', _id: { $in: parsed.data.audienceTenantIds }, status: 'active' }
    : { role: 'tenant', status: 'active' }
  
  User.find(targetFilter).then(tenants => {
    for (const tenant of tenants) {
      if (tenant.email) {
        sendEmail({
          to: tenant.email,
          subject: `Important Announcement: ${doc.title}`,
          html: `
            <p>Hello ${tenant.name},</p>
            <p>A new announcement has been posted by Plaza Management:</p>
            <h3>${doc.title}</h3>
            <p>${doc.body.replace(/\\n/g, '<br/>')}</p>
  ...
  }).catch(err => console.error('Failed to fetch tenants for announcement email', err))
